...
(function(){
  ...
  function escQ(v){return String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot',"'":'&#39;'}[m]))}
  ...
  function requestDate(text){
    const t=String(text||'');
    const labeled=t.match(/(?:기안\s*일자|요청\s*일|검사\s*(?:요청|의뢰)\s*일)[^0-9]{0,20}(20\d{2})\s*[.\-/년]\s*(\d{1,2})\s*[.\-/월]\s*(\d{1,2})/i);
    if(labeled)return normalizeDate(labeled[1],labeled[2],labeled[3]);
    const any=t.match(/\b(20\d{2})[.\-/](\d{1,2})[.\-/](\d{1,2})\b/);
    return any?normalizeDate(any[1],any[2],any[3]):'';
  }
  ...
  window.loadQualityPdf=async function(input){
    ...
    try{
      ...
      const d=document.getElementById('qDate');if(d&&!d.value)d.value=requestDate(result.text)||new Date().toISOString().slice(0,10);
