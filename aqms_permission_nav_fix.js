...
(function(){
 ...
 function utilityBottom(){
   const nav=document.getElementById('nav');if(!nav)return;ensureUtilityStyle();
   const aqms=nav.querySelector('[data-group="g-aqms"]');
   const doc=nav.querySelector('[data-group="g-doc"]');
   const admin=nav.querySelector('[data-group="g-admin"]');
   // Force final order: all business groups -> AQMS -> utility(Audit/Admin).
 ...
 setTimeout(maintain,150);setTimeout(maintain,500);setTimeout(maintain,1200);setInterval(maintain,1500);
